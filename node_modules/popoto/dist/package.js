...
export var jest = {"setupFiles":["jest-canvas-mock"],"moduleNameMapper":{"^d3$":"<rootDir>/node_modules/d3/dist/d3.min.js"}};
export var scripts = {"build":"rimraf dist && mkdir dist && json2module package.json > dist/package.js && node rollup.node && rollup -c --banner \"// Copyright (c) 2018 NHOGS Interactive.\" && npm run uglify","pretest":"npm run build","test":"jest --ci --coverage","uglify":"uglifyjs -b beautify=false,preamble=\"'// Copyright (c) 2018 NHOGS Interactive.'\" dist/popoto.js -c negate_iife=false -m -o dist/popoto.min.js && uglifycss css/popoto.css css/popoto-svg.css --output dist/popoto.min.css","update":"npx npm-check-updates -u"};
export var dependencies = {"d3":"^7.9.0"};
export var devDependencies = {"@babel/core":"^7.25.8","babel-jest":"^29.7.0","@babel/plugin-transform-modules-commonjs":"^7.25.7","jest":"^29.7.0","jest-canvas-mock":"^2.5.2","jest-environment-jsdom":"^29.7.0","json2module":"^0.0.3","rimraf":"^6.0.1","rollup":"^2.79.2","rollup-plugin-copy":"^3.5.0","rollup-plugin-node-resolve":"^5.2.0","uglify-js":"^3.19.3","uglifycss":"^0.0.29"};
